...
if (profileNamespace getVariable [format ["btc_hm_%1_db",worldName],false]) then {
	[[9],"btc_fnc_show_hint"] spawn BIS_fnc_MP;
	diag_log format ["DATA SAVE, count array in profileNamespace: count _cities_status %1 count _array_ho %2", count (profileNamespace getVariable [format ["btc_hm_%1_cities",_name],[]]), count (profileNamespace getVariable [format ["btc_hm_%1_ho",_name],[]])];
	diag_log format ["DATA SAVE, count array in profileNamespace: count _fobs %1 count _array_veh %2 count _array_obj %3", count (profileNamespace getVariable [format ["btc_hm_%1_fobs",_name],[]]), count (profileNamespace getVariable [format ["btc_hm_%1_vehs",_name],[]]), count (profileNamespace getVariable [format ["btc_hm_%1_objs",_name],[]])];
